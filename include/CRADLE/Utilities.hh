...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  inline double RandomFromDistribution(std::vector<std::vector<double> >& pd) {
    ...
    } while (q > pd[r*pd.size()][1]) ; //(q >= pd[(int)(r*(pd.size()-1)+0.5)][1]);
...
inline double GetBetaCorrections(int Z, int A, double Q, double E, int betaType, int Labs, bool advanced) { 
    ...
    //               * L0Correction
    //               * CCorrectionComponents
    //               * UCorrection
    //               * AtomicScreeningCorrection
    //               * RadiativeCorrection / RC_O1corr (only O2 & O3 RC)
    //               * RecoilCorrection
    ...
    W = std::max(W, 1.01); // Avoid W=1 which causes divergences in some corrections
    if (W > W0) {
    ...
    double g = 3. * std::log(PMASSC2/EMASSC2) 
               - 0.75
