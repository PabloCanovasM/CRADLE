...
inline double MBR(double E2, double K, double COS_GAMMA, double N1_K, double N1_N2, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    /*double EPS = (MF_2) + (std::pow(LAMBDA, 2)) * (MGT_2) ;
    double EPSa = (MF_2) - (std::pow(LAMBDA, 2)) * (MGT_2)/3. ;
    double a = EPSa/EPS ;*/
    
    return 16 * std::pow(FERMICONSTANT, 2) * std::pow( (MIMASSC2/EMASSC2) , 2 ) * std::pow(e, 2) * (1 * H0(E2, K, COS_GAMMA, MIMASSC2, MFMASSC2, mode) + a * H1(E2, K, COS_GAMMA, N1_K, N1_N2, MIMASSC2, MFMASSC2, mode)) * utilities::FermiFunction(Z, E2, R, betaType);
}

inline double M0(double E2, double C, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double E10 = delta(MIMASSC2, MFMASSC2, mode) - E2 ;
    double BETA = std::sqrt( (1. - 1./std::pow(E2, 2)) ) ;
    /*double EPS = (MF_2) + (std::pow(LAMBDA, 2)) * (MGT_2) ;
    double EPSa = (MF_2) - (std::pow(LAMBDA, 2)) * (MGT_2)/3. ; 
    double a = EPSa/EPS ;*/
    return 16. * std::pow(FERMICONSTANT, 2) * std::pow( MIMASSC2/EMASSC2 , 2 ) * E10 * E2 * (1 + a * BETA * C) * utilities::FermiFunction(Z, E2, R, betaType);
}

inline double Mtilde(double E2, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double E10 = delta(MIMASSC2, MFMASSC2, mode) - E2 ;
    double BETA = std::sqrt( (1. - 1./std::pow(E2, 2)) ) ;
    double N = 0.5 * log( (1. + BETA)/(1. - BETA) ) ;
    //double EPS = (MF_2) + (std::pow(LAMBDA, 2)) * (MGT_2) ;
    return - (FINESTRUCTURE/PI) * (16. * std::pow(FERMICONSTANT, 2) * ((1 - std::pow(BETA, 2))/BETA) * (N*std::pow( (MIMASSC2/EMASSC2) , 2 )*E10*E2*1 )) * utilities::FermiFunction(Z, E2, R, betaType);
}

inline double MVS(double E2, double C, double Cs,double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double E10 = delta(MIMASSC2, MFMASSC2, mode) - E2 ;
    double BETA = std::sqrt( (1. - 1./std::pow(E2, 2)) ) ;
    double N = 0.5 * log( (1. + BETA)/(1. - BETA) ) ;
    double omega = E10 * Cs ;
    ...
    return zVS * M0(E2, C, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) + Mtilde(E2, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
}
...
inline double WH(double E2, double K, double COS_GAMMA, double N1_K, double N1_N2, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double E1 = delta(MIMASSC2, MFMASSC2, mode) - E2 - K ; 
    double BETA = std::sqrt( (1. - 1./std::pow(E2, 2)) ) ;
    return ( K * BETA * E1 * E2 * MBR(E2, K, COS_GAMMA, N1_K, N1_N2, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) )/( std::pow(2, 13) * std::pow(PI, 8) * g_weight(E2, K, COS_GAMMA) * std::pow( (MIMASSC2/EMASSC2) , 2 )) ;
}

inline double rho_H(int n, double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double somme_rhoH = 0. ;
    ...
    for (int i = 0 ; i < n ; i++) {
        ...
        double wh = WH(E2, K, COS_GAMMA, N1_K, N1_N2, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
...
inline double WH_max(int n, double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double max = 0. ;
    ...
    for (int i = 0 ; i < n ; i++) {
        ...
        if (wh > max ) {
            if (std::isnan(wh) || std::isinf(wh)) {
                ...
                max = WH(E2, K, COS_GAMMA, N1_K, N1_N2, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
            }
        }
    }
    return max ;
...
// Equation 5.18
inline double w0(double E2, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double E10 = delta(MIMASSC2, MFMASSC2, mode) - E2 ;
    double BETA = std::sqrt( (1. - 1./std::pow(E2, 2)) ) ;
    //double EPS = (MF_2) + (std::pow(LAMBDA, 2)) * (MGT_2) ;
    double weight0 = ( std::pow(FERMICONSTANT, 2) * 1 * BETA * std::pow(E10, 2) * std::pow(E2, 2)  )/(2 * std::pow(PI, 3)) ;

    return weight0 * utilities::FermiFunction(Z, E2, R, betaType);
}

// Calcul du max de w0 pour Neumann Rejection
inline double w0_max(double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double w0max = 0 ;
    double n = 2000 ;

    double intervalle_E2 = (delta(MIMASSC2, MFMASSC2, mode))/(n-1) ;
    std::vector<double> tableau_E2(n); 
    for (int i=0 ; i<n ; i++) {
        tableau_E2[i] = 1 + i * intervalle_E2 ;

        if (w0max < w0(tableau_E2[i], a, MIMASSC2, MFMASSC2, Z, R, betaType, mode)) {
            w0max = w0(tableau_E2[i], a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
        }
    }
...
*/
inline double rho0(int n, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double b = delta(MIMASSC2, MFMASSC2, mode)-1 ;

    double h = b/(n+1) ; 
    double result = 0 ;
    for (int i=0 ; i<n ; i++) {
        if (!std::isnan(w0(1+h*i, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode))) {
        //if(w0(1+h*i, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode)<=0 or w0(1+h*i, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode)>=0 ) {
            
            double dx1 = 1+i*h ;
            double dx2 = dx1 + h ;

            result += (h)*w0(0.5*(dx1+dx2), a, MIMASSC2, MFMASSC2, Z, R, betaType, mode);
        }
    }
...
// Equation 5.20
inline double wVS(double E2, double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double E10 = delta(MIMASSC2, MFMASSC2, mode) - E2 ;
    double BETA = std::sqrt( (1. - 1./std::pow(E2, 2)) ) ;
    double N = 0.5 * log( (1. + BETA)/(1. - BETA) ) ; 
    double omega = E10 * Cs ;
    double zVS = (FINESTRUCTURE/PI) * (1.5 * log(PMASSC2/EMASSC2) + 2. *( (N/BETA) - 1. ) * log(2.*omega) + 2 * (N/BETA) * (1. - N) + (2./BETA) * (Spence_function(2. * BETA/(1. + BETA))) - (3./8.)) ;
    double weightVS = w0(E2, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) * (zVS - (FINESTRUCTURE * N/PI) * ( 1. - std::pow(BETA, 2))/BETA) ;
    
    return weightVS ;
}

// Calcul du max de wVS pour Neumann Rejection
inline double wVS_max(double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double wVSmax = 0 ;
    double n = 2000 ;
    ...
    for (int i=0 ; i<n ; i++) {
        ...
        if (wVSmax < wVS(tableau_E2[i], Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode)) {
            wVSmax = wVS(tableau_E2[i], Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
        }
    }
    return wVSmax ; 
}

inline double rhoVS(int n, double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double b = delta(MIMASSC2, MFMASSC2, mode)-1 ;
    double h = b/(n+1) ; 
    double result = 0 ;
    for (int i=0 ; i<n ; i++) {
        if(!std::isnan(wVS(1+h*i, Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode)) ) {

            double dx1 = 1+i*h ;
            double dx2 = dx1 + h ;
            result += (h)*wVS(0.5*(dx1+dx2), Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode);
...
// Equation 5.18 + 5.20
inline double w0VS(double E2, double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    return w0(E2, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) + wVS(E2, Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode);
}

// Calcul du max de w0VS pour Neumann Rejection (spectre)
inline double w0VS_max(double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double w0VSmax = 0 ;
    double n = 2000 ;
    ...
    for (int i=0 ; i<n ; i++) {
        ...
        if (w0VSmax < w0VS(tableau_E2[i], Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode)) {
            w0VSmax = w0VS(tableau_E2[i], Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
        }
    }
    return w0VSmax ;
}

inline double W0(double E2, double C, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double E10 = delta(MIMASSC2, MFMASSC2, mode) - E2 ;
    double BETA = std::sqrt( (1. - 1./std::pow(E2, 2)) ) ;
    return BETA * E10 * E2 * M0(E2, C, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode);
}
...
inline double W0_max(double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    ...
    for (int i=0 ; i<=n ; i++) {
        for (int j=0 ; j<=n ; j++) {
            if (W0max < W0(tableau_E2[i], tableau_cos[j], a, MIMASSC2, MFMASSC2, Z, R, betaType, mode)) {
                W0max = W0(tableau_E2[i], tableau_cos[j], a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
            }
        }
    }
    return W0max ;
...
inline double W0VS(double E2, double C, double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double E10 = delta(MIMASSC2, MFMASSC2, mode) - E2 ;
    double BETA = std::sqrt( (1. - 1./std::pow(E2, 2)) ) ;
    return BETA * E10 * E2 * ( M0(E2, C, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) + MVS(E2, C, Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ); 
}
...
inline double W0VS_max(double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    ...
    for (int i=0 ; i<=n ; i++) {
        for (int j=0 ; j<=n ; j++) {
            if (W0VSmax < W0VS(tableau_E2[i], tableau_cos[j], Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode)) {
                W0VSmax = W0VS(tableau_E2[i], tableau_cos[j], Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
            }
        }
    }
...
inline double PH(double Cs, double a, double MIMASSC2, double MFMASSC2, int Z, double R, int betaType, std::string mode) {
    double RHOH = rho_H(1000000, Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
    double RHO0 = rho0(2000, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
    double RHOVS = rhoVS(2000, Cs, a, MIMASSC2, MFMASSC2, Z, R, betaType, mode) ;
